...
public class RegistrationHttpPostTask {
	...
	QuestionnaireActivity start;
	HttpAsyncTask task;

	public RegistrationHttpPostTask(Context con, QuestionnaireActivity start, RegistrationDAO _registration) {
		context = con;
		this.start = start;
	...
	private class HttpAsyncTask extends AsyncTask<String, Void, String> {
		...
		private QuestionnaireActivity registrationActivity;
		private RegistrationDAO registration;

		public void setMainActivity(QuestionnaireActivity mainActivity) {
		...
		protected String doInBackground(String... urls) {
			...
			try {
				jsonObject.accumulate("q1_frequency", registration.getQ1_frequency());
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			try {
				jsonObject.accumulate("q2_readingtime", registration.getQ2_readingtime());
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			try {
				jsonObject.accumulate("q3_browsingstrategy", registration.getQ3_browsingstrategy());
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			try {
				jsonObject.accumulate("q4_readingstyle", registration.getQ4_readingstyle());
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		protected void onPostExecute(String result) {
			...
			result2 = result;
			try {
				registrationActivity.registerCompleted(result);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
